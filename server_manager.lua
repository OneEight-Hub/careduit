-- ==============================================================================
-- CDID HUB - DYNAMIC SERVER & PRIVATE SERVER MANAGER (FIXED CODE CAPTURE)
-- ==============================================================================
local ServerManager = {}
...
function ServerManager.Init(Window, Utils, Context)
	...
	-- ==============================================================================
	-- 3. DETEKSI & SINKRONISASI KODE INSTAN (VALIDATOR DIPERBAIKI)
	-- ==============================================================================
	local function IsValidServerCode(text)
		...
		-- Hanya tolak jika ini teks Ping / Region / FPS HUD
		...
		-- Format kode CDID adalah alfanumerik 6 - 32 karakter (Contoh: b95183187168276)
		if clean:len() >= 4 and clean:len() <= 35 then
			return true
		end
	...
	local function ApplyServerCode(code)
		...
		print("🔑 [Private Server] Kode ASLI berhasil tertangkap:", activeServerCode)
	...
	local function HookServerLabelInstance(labelInstance)
		...
		-- Baca teks awal jika valid
		if IsValidServerCode(labelInstance.Text) then
			ApplyServerCode(labelInstance.Text)
		end

		-- Pasang listener saat teks berubah (misal habis tekan GENERATE)
	...
	local function ScanExactPrivateServerFrame()
		...
		-- 1. Cari langsung dari frame UI Private Server resmi
		for _, inst in ipairs(pGui:GetDescendants()) do
			if inst:IsA("TextLabel") and inst.Name == "ServerLabel" then
				HookServerLabelInstance(inst)
	...
	PrivateServerSection:Button({
		...
		Callback = function()
			-- Jika variabel kosong, paksa scan label di layar
	...
	PrivateServerSection:Button({
		...
		Callback = function()
			print("🔄 [Private Server] Request generate/ambil kode server...")
			if CDID_Network then
			...
			task.spawn(function()
				for _ = 1, 6 do
					task.wait(0.5)
					if ScanExactPrivateServerFrame() then break end
				end
			end)
		end
	})
	...
	-- Auto Scan saat script pertama dibuka
	task.spawn(function()
		task.wait(1.0)
		ScanExactPrivateServerFrame()
	end)
